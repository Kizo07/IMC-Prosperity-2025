...
PARAMS = {
    ...
    Product.KELP: {
        "fair_value": 2000,
        "ewma_beta": 0.15, # Increased beta for more stability
        "clear_width": 0,
        "rolling_window": 100,
        "spread_multiplier": 0.9,  # Lower spread multiplier for tighter spreads
        "min_spread": 2,
        "max_spread": 4,           # Reduced max spread for less volatility
        "position_scale": 0.4,     # Reduced position scale for smaller orders when position grows
        "vol_window": 20,          # Increased volatility window for more stable volatility estimate
        "order_skew_threshold": 0.3, # Higher threshold to make skewing less aggressive
        "mean_reversion_strength": 0.3, # Controls how strongly we react to mean reversion signals
        "max_signal_impact": 0.3,  # Limits how much mean reversion can move our fair value
        "aggressive_order_threshold": 0.7 # Only place aggressive orders with very strong signals
    },
    Product.SQUID_INK: {
        "fair_value": 2000,
        "ewma_beta": 0.3, # Increased beta for more stability
        "clear_width": 0,
        "rolling_window": 100,
        "spread_multiplier": 0.9,  # Lower spread multiplier for tighter spreads
        "min_spread": 2,
        "max_spread": 5,           # Reduced max spread
        "position_scale": 0.5,     # Reduced position scale
        "vol_window": 25,          # Increased volatility window for more stable estimate
        "order_skew_threshold": 0.35, # Higher threshold to make skewing less aggressive
        "base_order_size": 12      # Smaller base order size
    }
}
...
class Trader:
    ...
    def squid_ink_market_making(self,
        ...
        """
        Advanced market making strategy for SQUID_INK with reduced volatility
        and negative autocorrelation signals
        """
        if fair_value is None:
            return [], 0, 0
        ...
        # Get autocorrelation signals - same approach as KELP but possibly different patterns
        corr_strength, bid_adjustment, ask_adjustment = self.analyze_price_autocorrelation(
            product, traderObject, fair_value)
            
        # If we detect meaningful negative autocorrelation, use it to adjust our prices
        if corr_strength < -0.2:  # Only use when we detect genuine negative autocorrelation
            # Apply price adjustments from autocorrelation analysis
            adjusted_fair_value = fair_value
            adjusted_bid = fair_value - (optimal_spread / 2) + bid_adjustment
            adjusted_ask = fair_value + (optimal_spread / 2) + ask_adjustment
        else:
            # Fall back to mean reversion approach with conservative impact
            # Get more history for price analysis
            prices = list(traderObject["price_history"][product])
            if len(prices) >= 5:
                # Convert to numpy array for calculations
                price_array = np.array(prices)
                # Calculate rolling mean for mean reversion target
                rolling_mean = np.mean(price_array[-5:])
                
                # Calculate reversion strength - pull toward the mean
                reversion_factor = 0.2  # Conservative reversion strength
                adjusted_fair_value = fair_value * (1 - reversion_factor) + rolling_mean * reversion_factor
                
                # Calculate bid and ask prices
                half_spread = optimal_spread / 2
                adjusted_bid = adjusted_fair_value - half_spread
                adjusted_ask = adjusted_fair_value + half_spread
            else:
                adjusted_fair_value = fair_value
                half_spread = optimal_spread / 2
                adjusted_bid = fair_value - half_spread
                adjusted_ask = fair_value + half_spread
        
        # Apply a smaller imbalance adjustment
        if abs(imbalance) > self.params[product]["order_skew_threshold"]:
            imbalance_adjustment = imbalance * optimal_spread / 5  # Smaller impact
            adjusted_bid += imbalance_adjustment
            adjusted_ask += imbalance_adjustment
            
        # Round to integers
        bid_price = int(adjusted_bid)
        ask_price = int(adjusted_ask)
        ...
        base_order_size = self.params[product]["base_order_size"]  # Default size from params
        
        # If we detect strong negative autocorrelation, increase order sizes
        # This is to capitalize on the statistical edge
        autocorr_scale = 1.0
        if corr_strength < -0.3:  # Strong negative autocorrelation
            autocorr_scale = 1.15  # Increase sizes by 15% (slightly more conservative than KELP)
        elif corr_strength < -0.5:  # Very strong negative autocorrelation
            autocorr_scale = 1.3   # Increase sizes by 30% (more conservative than KELP)
            
        # Calculate final order sizes
        adjusted_order_size = max(1, int(base_order_size * position_scale * autocorr_scale))
        
        # More conservative order size scaling based on position
        buy_size = adjusted_order_size
        sell_size = adjusted_order_size
        
        if position > 0:
            # More aggressive sell scaling when long
            sell_size = int(adjusted_order_size * (1 + min(position_ratio * 0.7, 0.5)))
            buy_size = int(adjusted_order_size * (1 - min(position_ratio * 0.7, 0.7)))
        elif position < 0:
            # More aggressive buy scaling when short
            sell_size = int(adjusted_order_size * (1 - min(abs(position_ratio) * 0.7, 0.7)))
            buy_size = int(adjusted_order_size * (1 + min(abs(position_ratio) * 0.7, 0.5)))
            
        # Ensure minimum sizes
        buy_size = max(1, buy_size)
        sell_size = max(1, sell_size)
            
        # Add price floor/ceiling to prevent extreme prices
        prices = list(traderObject["price_history"][product])
        if len(prices) > 10:
            price_array = np.array(prices)
            avg_price = np.mean(price_array[-10:])
            max_deviation = avg_price * 0.02  # Maximum 2% deviation from recent average
            
            bid_price = max(bid_price, int(avg_price - max_deviation))
            ask_price = min(ask_price, int(avg_price + max_deviation))
    ...
    def kelp_market_making(self,
        ...
        """
        Advanced market making strategy for KELP with negative autocorrelation
        Leverages statistical signals to predict short-term price movements
        """
        if fair_value is None:
            return [], 0, 0
        ...
        # Get autocorrelation signals - new approach that examines price patterns
        corr_strength, bid_adjustment, ask_adjustment = self.analyze_price_autocorrelation(
            product, traderObject, fair_value)
        ...
        if corr_strength < -0.2:  # Only use when we detect genuine negative autocorrelation
            ...
            # Fall back to mean reversion approach with reduced impact
            mean_rev_signal, expected_movement = self.mean_reversion_signal(product, traderObject, fair_value)
            adjusted_fair_value = fair_value + expected_movement * 0.4  # Reduced impact
        ...
        # Further adjust fair value slightly based on imbalance if significant
        if abs(imbalance) > self.params[product]["order_skew_threshold"]:
            imbalance_adjustment = imbalance * optimal_spread / 6  # Smaller impact
            adjusted_bid += imbalance_adjustment
            adjusted_ask += imbalance_adjustment
        ...
        # Base order size calculation
        base_order_size = 18  # Default size
        
        # If we detect strong negative autocorrelation, increase order sizes
        # This is to capitalize on the statistical edge
        autocorr_scale = 1.0
        if corr_strength < -0.3:  # Strong negative autocorrelation
            autocorr_scale = 1.2  # Increase sizes by 20%
        elif corr_strength < -0.5:  # Very strong negative autocorrelation
            autocorr_scale = 1.4  # Increase sizes by 40%
            
        # Calculate final order sizes
        adjusted_order_size = max(1, int(base_order_size * position_scale * autocorr_scale))
        
        # Skew order sizes based on position
        buy_size = adjusted_order_size
        sell_size = adjusted_order_size
        ...
        if position > 0:
            ...
            buy_size = int(adjusted_order_size * (1 + abs(position_ratio) * 0.5))
            sell_size = int(adjusted_order_size * (1 - abs(position_ratio) * 0.5))
        ...
        if len(prices) > 10:
            price_array = np.array(prices)
            avg_price = np.mean(price_array[-10:])
            max_deviation = avg_price * 0.015  # Maximum 1.5% deviation
    ...
    def mean_reversion_signal(self, product: str, traderObject: dict, current_price: float) -> tuple[float, float]:
        """
        Calculate mean reversion signal using numpy for efficient calculations
        Returns a signal strength (-1 to +1) and the expected price movement
        Modified to be more conservative with reduced volatility
        """
        if "price_history" not in traderObject:
            traderObject["price_history"] = {}
        ...
        # Need at least 5 prices to calculate a reliable mean reversion signal
        ...
        # Calculate returns (using numpy's diff for efficiency)
        returns = np.diff(prices)
        
        # Get the most recent returns - using more history for stability
        last_returns = returns[-3:] if len(returns) >= 3 else returns
        
        # Take the average of recent returns for more stability
        avg_return = np.mean(last_returns)
        
        # Calculate volatility for normalization
        vol_window = min(20, len(returns))
        recent_returns_vol = np.std(returns[-vol_window:]) if vol_window > 1 else 0.1
        
        # If volatility is too low, use a minimum value to avoid extreme signals
        recent_returns_vol = max(recent_returns_vol, current_price * 0.001)
        
        # Mean reversion signal - opposite of the average return direction
        # But dampened for lower volatility
        signal_strength = self.params[product].get("mean_reversion_strength", 0.3)
        max_impact = self.params[product].get("max_signal_impact", 0.3)
        
        # Normalize by volatility and cap signal
        raw_signal = -np.sign(avg_return) * min(abs(avg_return / recent_returns_vol), 1.0)
        
        # Apply dampening to reduce signal volatility
        signal = raw_signal * signal_strength
        
        # Calculate expected price movement - more conservative estimate
        expected_movement = -avg_return * max_impact
        
        return signal, expected_movement
    ...
    def analyze_price_autocorrelation(self, product: str, traderObject: dict, current_price: float) -> tuple[float, float, float]:
        ...
        # Need at least 10 prices for meaningful autocorrelation analysis
        ...
        # Recent price movement direction (for making market making adjustments)
        recent_movement = returns[-1] if returns.size > 0 else 0
        
        # Calculate appropriate adjustments based on negative autocorrelation
        # When prices just went up, if there's negative autocorrelation, we expect it to go down next
        # So we might want to adjust our ask lower to get filled before the drop
        # Similarly, when prices just went down, we expect them to go up, so adjust bid higher
        adjustment_factor = abs(correlation_strength) * 0.5  # Scale factor for adjustments
        
        # If price just went up, adjust ask lower to sell before expected drop
        # If price just went down, adjust bid higher to buy before expected rise
        if recent_movement > 0:  # Price just went up
            bid_adjustment = 0  # Keep bid as is
            ask_adjustment = -recent_movement * adjustment_factor  # Lower ask slightly
        else:  # Price just went down or stayed the same
            bid_adjustment = -recent_movement * adjustment_factor  # Raise bid slightly
            ask_adjustment = 0  # Keep ask as is
