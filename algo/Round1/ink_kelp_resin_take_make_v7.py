...
PARAMS = {
    ...
    Product.KELP: {
        ...
        "clear_width": 0,
        "rolling_window": 100,  # Window for price history
        "spread_multiplier": 1.3,  # Increased spread multiplier
        "min_spread": 3,           # Increased minimum spread
        "max_spread": 7,           # Increased maximum spread
        "position_scale": 0.8,     # Increased position scaling to reduce size faster
        "vol_window": 15,          # Window for calculating spread
        "order_skew_threshold": 0.2, # Reduced threshold for skewing orders
        # Momentum following parameters - use autocorrelation directly (not inverted)
        "auto_correlation_window": 20,  # Window for measuring autocorrelation
        "momentum_strength": 0.7,  # Strength of momentum signal
        "lag1_weight": 0.7,  # Weight for 1-period lag (stronger correlation)
        "lag2_weight": 0.3,  # Weight for 2-period lag (weaker correlation)
        # Cross-asset correlation parameters
        "squid_correlation": 0.31,  # Correlation with squid ink
        "squid_signal_weight": 0.25,  # Weight for squid ink signal
        "max_position_imbalance": 0.5  # Maximum allowed position imbalance between kelp and squid
    },
    Product.SQUID_INK: {
        "fair_value": 2000,
...
class Trader:
    ...
    def calculate_book_imbalance(self, order_depth):
        ...
        sell_volume = abs(sum(order_depth.sell_orders.values()))
    ...
    def kelp_market_making(self,
        ...
        """
        Advanced market making strategy for KELP that leverages:
        1. Strong negative autocorrelation (-0.48 at lag 1, -0.27 at lag 2)
        2. Correlation with SQUID_INK (0.31)
        3. Does the opposite of what signals suggest (shorts when should go long, goes long when should short)
        """
        if fair_value is None:
            return [], 0, 0
        ...
        if "price_history" not in traderObject:
            traderObject["price_history"] = {}
        if product not in traderObject["price_history"]:
            traderObject["price_history"][product] = deque(maxlen=self.params[product]["rolling_window"])
        
        # Store return history for autocorrelation
        if "return_history" not in traderObject:
            traderObject["return_history"] = {}
        if product not in traderObject["return_history"]:
            traderObject["return_history"][product] = deque(maxlen=self.params[product]["auto_correlation_window"])
        
        # Calculate current return if we have previous price
        prev_price = traderObject["last_price"].get(product)
        if prev_price is not None and prev_price != 0:
            current_return = (fair_value - prev_price) / prev_price
            traderObject["return_history"][product].append(current_return)
        else:
            current_return = 0
        
        # Store current price for next iteration
        traderObject["last_price"][product] = fair_value
        ...
        # Current market spread
        current_spread = best_ask - best_bid if best_bid is not None and best_ask is not None else 3
        ...
        # Calculate momentum signal (same as before)
        momentum_signal = 0
        returns = list(traderObject["return_history"][product])
        
        if len(returns) >= 3:
            # Use lag 1 and lag 2 returns to predict next return
            lag1_return = returns[-1]
            lag2_return = returns[-2] if len(returns) > 1 else 0
            
            # Calculate momentum signal
            momentum_signal = (lag1_return * self.params[product]["lag1_weight"] + 
                              lag2_return * self.params[product]["lag2_weight"])
            
            # Scale the signal based on momentum strength parameter
            momentum_signal *= self.params[product]["momentum_strength"]
        
        # Cross-asset correlation signal with SQUID_INK
        squid_correlation_signal = 0
        if Product.SQUID_INK in traderObject["return_history"] and len(traderObject["return_history"][Product.SQUID_INK]) > 0:
            # Get the most recent squid ink return
            squid_return = traderObject["return_history"][Product.SQUID_INK][-1]
            
            # Positive correlation means we expect same direction
            squid_correlation_signal = squid_return * self.params[product]["squid_correlation"]
        
        # Adjust fair value based on signals
        adjusted_fair_value = fair_value
        
        # Combine signals: momentum is primary, book imbalance secondary, squid correlation tertiary
        total_signal = (momentum_signal * 0.6 +
                       imbalance * 0.15 * optimal_spread +
                       squid_correlation_signal * self.params[product]["squid_signal_weight"])
        
        adjusted_fair_value += total_signal * fair_value
        ...
        # Increase order size when we have a strong momentum signal
        if abs(momentum_signal) > 0.0005:
            # Scale up order size based on signal strength
            signal_strength = min(1.5, abs(momentum_signal) * 1000)
            adjusted_order_size = int(adjusted_order_size * signal_strength)
        
        # Skew order sizes based on signals - BUT INVERTED
        buy_size = adjusted_order_size
        sell_size = adjusted_order_size
        
        # INVERT SIGNALS: If we have a strong momentum signal, skew orders in OPPOSITE direction
        if momentum_signal > 0:  # Positive momentum suggests rise, but we do opposite
            buy_size = int(adjusted_order_size * 0.7)   # BUY LESS (opposite)
            sell_size = int(adjusted_order_size * 1.5)  # SELL MORE (opposite)
        elif momentum_signal < 0:  # Negative momentum suggests fall, but we do opposite
            buy_size = int(adjusted_order_size * 1.5)   # BUY MORE (opposite)
            sell_size = int(adjusted_order_size * 0.7)  # SELL LESS (opposite)
        
        # Check position balance with squid ink (due to correlation) - OPPOSITE BEHAVIOR
        squid_position = traderObject.get("positions", {}).get(Product.SQUID_INK, 0)
        kelp_position = position
        
        # Update position tracking in traderObject for next iteration
        if "positions" not in traderObject:
            traderObject["positions"] = {}
        traderObject["positions"][product] = kelp_position
        
        # Calculate position imbalance and do OPPOSITE of correlation suggestion
        if abs(squid_position) > 10 and abs(kelp_position) > 10:
            # If correlation is positive (0.31), positions should be in same direction,
            # but we're doing the opposite
            positions_aligned = (squid_position * kelp_position > 0)
            
            if positions_aligned:  # If aligned, we want to make them opposite
                if kelp_position > 0 and squid_position > 0:
                    # Reduce kelp position (sell more, buy less)
                    sell_size = int(sell_size * 1.3)
                    buy_size = int(buy_size * 0.7)
                elif kelp_position < 0 and squid_position < 0:
                    # Increase kelp position (buy more, sell less)
                    buy_size = int(buy_size * 1.3)
                    sell_size = int(sell_size * 0.7)
        
        # Calculate bid and ask prices - BUT INVERTED
        half_spread = optimal_spread / 2
        
        # Adjust spread based on momentum signal - BUT INVERTED
        if momentum_signal > 0:  # Expect continued rise, but we bet against it
            bid_price = int(adjusted_fair_value - half_spread * 1.3)  # Wider bid (opposite)
            ask_price = int(adjusted_fair_value + half_spread * 0.7)  # Tighter ask (opposite)
        elif momentum_signal < 0:  # Expect continued fall, but we bet against it
            bid_price = int(adjusted_fair_value - half_spread * 0.7)  # Tighter bid (opposite)
            ask_price = int(adjusted_fair_value + half_spread * 1.3)  # Wider ask (opposite)
        else:
            bid_price = int(adjusted_fair_value - half_spread)
            ask_price = int(adjusted_fair_value + half_spread)
