...
PARAMS = {
    ...
    Product.SQUID_INK: {
        ...
        "mean_reversion_threshold": 0.0004,  # Return threshold to trigger mean reversion strategy
        "mean_reversion_factor": 1.5,        # How strongly to fade the move (higher = more aggressive)
        "mean_reversion_size_multiplier": 1.3 # Multiply order size when mean reversion expected
    }
}
...
class Trader:
    ...
    def rolling_mm_mid_quotes(
        ...
        if product not in traderObject["rolling_mid_quotes"]:
            traderObject["rolling_mid_quotes"][product] = []
        ...
        if len(rolling_prices) >= rolling_window:
            rolling_prices.pop(0)
        
        rolling_prices.append(mm_mid)
        
        traderObject["rolling_mid_quotes"][product] = rolling_prices
    ...
    def ink_fair_value(
        ...
        if len(kelp_prices) <= kelp_lag:
            return None
        
        #signal = kelp_prices[0]
        mean_signal = sum(kelp_prices) / len(kelp_prices)
    ...
    def calculate_optimal_spread(self, product, traderObject, current_spread):
        ...
        if product not in traderObject["price_history"]:
            traderObject["price_history"][product] = []
        ...
        recent_changes = price_changes[-min(len(price_changes), self.params[product]["vol_window"]):]
        ...
        try:
            ...
            volatility = (sum((x - mean) ** 2 for x in recent_changes) / len(recent_changes)) ** 0.5
    ...
    def squid_ink_market_making(self,
        ...
        """
        Advanced market making strategy for SQUID_INK combined with mean reversion
        that takes advantage of the discrete nature of price movements and expected reversals
        """
        orders = []
        
        # Initialize a returns tracker if it doesn't exist
        if "returns" not in traderObject:
            traderObject["returns"] = {}
        if product not in traderObject["returns"]:
            traderObject["returns"][product] = []
        ...
        # Calculate return if we have previous prices
        current_return = None
        if len(traderObject["price_history"][product]) > 0:
            prev_price = traderObject["price_history"][product][-1]
            current_return = (fair_value - prev_price) / prev_price if prev_price != 0 else 0
            traderObject["returns"][product].append(current_return)
            # Limit the size of returns history
            if len(traderObject["returns"][product]) > 100:
                traderObject["returns"][product].pop(0)
        
        # Add current price to history
        traderObject["price_history"][product].append(fair_value)
        if len(traderObject["price_history"][product]) > 100:
            traderObject["price_history"][product].pop(0)
            
        # Get best bid and ask
        best_bid, best_ask, _, _ = self.best_orders(product, order_depth)
        
        # Current market spread
        current_spread = best_ask - best_bid if best_bid and best_ask else 2
        ...
        # Estimate mid price for next tick considering mean reversion
        estimated_next_mid = fair_value  # Default value
        mean_reversion_active = False
        reversion_multiplier = 1.0
        mean_reversion_sign = 0  # 0: no reversion, 1: positive expected return, -1: negative expected return
        
        # Apply mean reversion logic if we have return data
        if current_return is not None:
            threshold = self.params[product]["mean_reversion_threshold"]
            
            # If we detect a significant price move, expect reversion
            if abs(current_return) > threshold:
                mean_reversion_active = True
                mean_reversion_sign = -1 if current_return > 0 else 1  # Opposite of the current move
                reversion_factor = self.params[product]["mean_reversion_factor"]
                
                # Calculate expected reversion amount (opposite to the recent return)
                reversion = -current_return * reversion_factor * fair_value
                estimated_next_mid = fair_value + reversion
                
                # Adjust the multiplier for order sizes
                reversion_multiplier = self.params[product]["mean_reversion_size_multiplier"]
        
        # Adjust fair value based on order book imbalance (still useful alongside mean reversion)
        if abs(imbalance) > self.params[product]["order_skew_threshold"]:
            estimated_next_mid += imbalance * optimal_spread / 3
        
        # Use our estimated next mid price as the fair value for placing orders
        fair_value = estimated_next_mid
        ...
        # If mean reversion is active, adjust order sizes
        if mean_reversion_active:
            # Increase the size of orders in the direction of expected reversion
            if mean_reversion_sign > 0:  # Expecting price to go up
                buy_size = int(adjusted_order_size * reversion_multiplier)
                sell_size = adjusted_order_size
            elif mean_reversion_sign < 0:  # Expecting price to go down
                buy_size = adjusted_order_size
                sell_size = int(adjusted_order_size * reversion_multiplier)
            else:
                buy_size = adjusted_order_size
                sell_size = adjusted_order_size
        else:
        ...
        # Further skew order sizes based on current position
        if position > 0:
            # If we're long, increase sell size and decrease buy size
            sell_size = int(sell_size * (1 + position_ratio * 0.5))
            buy_size = int(buy_size * (1 - position_ratio * 0.5))
        elif position < 0:
            # If we're short, increase buy size and decrease sell size
            buy_size = int(buy_size * (1 - position_ratio * 0.5))
            sell_size = int(sell_size * (1 + position_ratio * 0.5))
            
        # Calculate bid and ask prices based on our estimated fair value
        half_spread = optimal_spread / 2
        
        # If mean reversion is active, skew our quotes to capitalize on expected move
        if mean_reversion_active:
            if mean_reversion_sign > 0:  # Expecting price to go up
                # Place bid closer to fair value (tighter) to get filled before move up
                # Place ask further from fair value (wider) to sell at better prices after move
                bid_price = int(fair_value - half_spread * 0.8)  # Tighter bid
                ask_price = int(fair_value + half_spread * 1.2)  # Wider ask
            else:  # Expecting price to go down
                # Place bid further from fair value (wider) to buy at better prices after move
                # Place ask closer to fair value (tighter) to get filled before move down
                bid_price = int(fair_value - half_spread * 1.2)  # Wider bid
                ask_price = int(fair_value + half_spread * 0.8)  # Tighter ask
        # Otherwise use order book imbalance for skewing
            ...
            bid_price = int(fair_value - half_spread)
            ask_price = int(fair_value + half_spread)
    ...
    def kelp_market_making(self,
        ...
        if abs(imbalance) > self.params[product]["order_skew_threshold"]:
            fair_value += imbalance * optimal_spread / 3  # Less aggressive than SQUID_INK
        ...
        if imbalance > self.params[product]["order_skew_threshold"]:
            bid_price = int(fair_value - half_spread * 0.85)  # Tighter bid
            ask_price = int(fair_value + half_spread * 1.15)  # Wider ask
        elif imbalance < -self.params[product]["order_skew_threshold"]:
            bid_price = int(fair_value - half_spread * 1.15)  # Wider bid
            ask_price = int(fair_value + half_spread * 0.85)  # Tighter ask
        else:
            bid_price = int(fair_value - half_spread)
            ask_price = int(fair_value + half_spread)
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        if Product.KELP in state.order_depths:
            ...
            # Use our advanced market making strategy for KELP instead of the previous approach
        ...
        if Product.SQUID_INK in state.order_depths:
            ...
            # Use our advanced market making strategy for SQUID_INK instead of the previous approach
