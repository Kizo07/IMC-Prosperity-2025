...
class Trader:
    ...
    def __init__(self, params=None):
        ...
        self.LIMIT = {Product.RAINFOREST_RESIN: 50, Product.KELP: 50, Product.SQUID_INK: 50,
                      Product.PB1: 60, Product.PB2: 100}
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        if Product.PB1 in state.order_depths:
            pb1_position = state.position.get(Product.PB1, 0)
            pb1_order_depth = state.order_depths[Product.PB1]
            
            pb1_mm_bid, pb1_mm_ask, _, _ = self.mm_orders(Product.PB1, pb1_order_depth)
            pb1_mm_mid = (pb1_mm_bid + pb1_mm_ask) / 2
        
            pb1_bid, pb1_ask, _, _ = self.best_orders(Product.PB1, pb1_order_depth)    
                

            # strongly market neutral market making
            pb1_make_orders, _, _ = self.make_orders(Product.PB1,
                                                        pb1_order_depth,  # Product-specific OrderDepth
                                                        pb1_mm_mid,
                                                        pb1_position,
                                                        self.LIMIT[Product.PB1],
                                                        desired_edge = 5,
                                                        ...
                                                        max_deviation = 10,
                                                        take_buy_quantity = 0,
                                                        take_sell_quantity = 0,
                                                        desired_position = 0
                                                        )
            

            # clear orders for 0 EV
            pb1_clear_orders, _, _ = self.clear_orders(Product.PB1,
                                                        pb1_order_depth,
                                                        pb1_mm_mid,
                                                        pb1_position,
                                                        self.LIMIT[Product.PB1],
        ...
        if Product.PB2 in state.order_depths:
            pb2_position = state.position.get(Product.PB2, 0)
            pb2_order_depth = state.order_depths[Product.PB2]
            
            pb2_mm_bid, pb2_mm_ask, _, _ = self.mm_orders(Product.PB2, pb2_order_depth)
            pb2_mm_mid = (pb2_mm_bid + pb2_mm_ask) / 2
        
            pb2_bid, pb2_ask, _, _ = self.best_orders(Product.PB2, pb2_order_depth)    
                
            
            
            pb2_make_orders, _, _ = self.make_orders(Product.PB2,
                                                        pb2_order_depth,  # Product-specific OrderDepth
                                                        pb2_mm_mid,
                                                        pb2_position,
                                                        self.LIMIT[Product.PB2],
                                                        desired_edge = 3,
                                                        default_order_size = 10,
                                                        disregard_edge = 2.5,
                                                        direction_sensitive= True,
            ...
            pb2_clear_orders, _, _ = self.clear_orders(Product.PB2,
                                                        pb2_order_depth,
                                                        pb2_mm_mid,
                                                        pb2_position,
                                                        self.LIMIT[Product.PB2],
        ...
        #result[Product.PB1] = (pb1_clear_orders + pb1_make_orders)
        #result[Product.PB2] = (pb2_clear_orders + pb2_make_orders)
